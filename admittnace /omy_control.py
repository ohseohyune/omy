...
try:
    with mj_view.launch_passive(mjc_model, mjc_data) as viewer:
        while viewer.is_running():
            ...
            tau_g = pin.computeGeneralizedGravity(pin_model, pin_data, q_mjc)  # length nv
            print("tau_g:", np.round(tau_g, 3))

            dq_g = gravity_gain * tau_g
            print("dq_g:", np.round(dq_g, 3))

            print("delta_q:", np.round(delta_q, 3))

            # 제어 입력 = 어드미턴스 속도 + 중력 보상 속도
            qvel_cmd = delta_q + dq_g

            print("mjc gravity:", mjc_model.opt.gravity)
            print("q_mjc:", np.round(q_mjc, 3))
